...
class CustomDiffusionDataset(Dataset):
    ...
    def __init__(
        ...
        if len(self.instance_images_path)>4:
            self.instance_images_path = self.instance_images_path[:4]
    ...
    def __getitem__(self, index):
        example = {}
        instance_image_path, instance_prompt = self.instance_images_path[index % self.num_instance_images]
...
def main(args):
    ...
    for name, _ in unet.attn_processors.items():
        ...
        cross_attention_dim = None if layer_name.endswith("attn1") else unet.config.cross_attention_dim

        if layer_name.startswith("mid_block"):
            hidden_size = unet.config.block_out_channels[-1]
        elif layer_name.startswith("up_blocks"):
            block_id = int(layer_name.split(".")[1])
            hidden_size = list(reversed(unet.config.block_out_channels))[block_id]
        elif layer_name.startswith("down_blocks"):
            block_id = int(layer_name.split(".")[1])
            hidden_size = unet.config.block_out_channels[block_id]
        else:
            raise ValueError(f"Unexpected block name in {layer_name}")

        if layer_name in target_trainable_layers:
            processor = attention_class(
                train_kv=train_kv,
        ...
        custom_diffusion_attn_procs[name] = processor
    ...
    # Prepare for PGD
    pertubed_images = [Image.open(i[0]).convert("RGB").resize((args.resolution, args.resolution)) for i in train_dataset.instance_images_path]
